...
> 由 `hdc` 在 HarmonyOS 模拟器（`Huawei_2in1`，应用窗口最大化）上实时抓取，界面语言简体中文。
...
>
> 之所以是 2-in-1 而不是平板：侧栏的断点是 **1440vp**（照搬上游 `getWindowBreakpoint()`）。
> 2-in-1 窗口最大化后是 **1728vp**，侧栏是常驻的三栏之一；平板上只有 **1138vp**，
> 侧栏退化为覆盖列表的抽屉、卡片列也会被挤扁 —— 那是上游的响应式规则，不是缺陷。
...
| 侧栏 + 列表 + 阅读器（三栏） | 5 种布局之一 | 标题正则 `[软游]` 命中 9 篇 |
